...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const contextData = getWebGLContext(activeCanvas);
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(gl, (gl as any).RGBA16F, gl.RGBA, halfFloatTexType, supportLinearFiltering);
    ...
    function getSupportedFormat(gl: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number, supportLinearFiltering: any): any {
      if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        switch (internalFormat) {
          case (gl as any).R16F:
            return getSupportedFormat(gl, (gl as any).RG16F, (gl as any).RG, type, supportLinearFiltering);
          case (gl as any).RG16F:
            return getSupportedFormat(gl, (gl as any).RGBA16F, gl.RGBA, type, supportLinearFiltering);
          default:
            return null;
        }
      }
    ...
    function drawDisplay(target: any) {
      ...
      if (config.SHADING) {
        gl.uniform2f(displayMaterial.uniforms.texelSize as any, 1.0 / width, 1.0 / height);
      }
      gl.uniform1i(displayMaterial.uniforms.uTexture as any, dye.read.attach(0));
      blit(target);
    }
    ...
    // Hex and color conversion helper functions
